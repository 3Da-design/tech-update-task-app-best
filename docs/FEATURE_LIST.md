...
| データベース | PostgreSQL（Docker Compose・CI とも同一） |
...
JSON でタスクの CRUD を提供します。`API\TaskController` 内で **認証ユーザーのタスク** のみを操作します（テストでは `actingAs` で認証を模擬）。
...
| レイヤ | 主なクラス・ファイル |
|--------|----------------------|
| Web コントローラ | `App\Http\Controllers\Web\TaskController`（業務ロジック・DB 直アクセス） |
| API コントローラ | `App\Http\Controllers\API\TaskController`（同上・ロジック重複） |
